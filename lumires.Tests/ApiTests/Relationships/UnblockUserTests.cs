...
using lumires.Api.Features.Relationships.UnblockUser;
...
internal sealed class UnblockUserTests
{
    ...
    [Test]
    public async Task UnblockUser_Should_Return_204_When_Unblocking_Self()
    {
        // Arrange
    ...
    [Test]
    public async Task UnblockUser_Should_Return_404_When_Target_User_Not_Found()
    {
        // Arrange
        SetupData([], []);
        var ep = CreateEndpoint();
    ...
    [Test]
    public async Task UnblockUser_Should_Return_204_And_Remove_When_Block_Exists()
    {
        // Arrange
        var userId = Guid.NewGuid();
        ...
        // Assert
        ep.HttpContext.Response.StatusCode.Should().Be(204);
        _dbContextMock.Verify(x => x.Relationships.Remove(blockRelationship), Times.Once);
        _dbContextMock.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }

    [Test]
    public async Task UnblockUser_Should_Return_204_And_Not_Remove_When_Follow_Exists()
    {
        // Arrange
        var userId = Guid.NewGuid();
        var targetUser = new User(Guid.NewGuid(), "target", "target@mail.com");
    ...
    [Test]
    public async Task UnblockUser_Should_Return_204_When_No_Relationship_Exists()
    {
        // Arrange
        var userId = Guid.NewGuid();
        var targetUser = new User(Guid.NewGuid(), "target", "target@mail.com");
