...
[UsedImplicitly]
internal sealed record Command(Guid ThreadId, Guid ReplyId);
    ...
    : Endpoint<Command, EmptyResponse>
{
    ...
    {
        Delete("/threads/{threadId:guid}/replies/{replyId:guid}");
    ...
    public override async Task HandleAsync(Command command, CancellationToken ct)
    {
        ...
        var result = await dataAccess.DeleteThreadCommentAsync(command, currentUserId, userRole, ct);
