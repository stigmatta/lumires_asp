...
internal class DataAccess(IAppDbContext db) : IDataAccess
{
    ...
    internal async Task<Response?> GetMostReviewedFilmByDirector(
        int directorId, string lang, Guid currentUserId, CancellationToken ct)
    {
        return await db.FilmDirectors
            .AsNoTracking()
            .Where(x => x.Person.ExternalId == directorId &&
                        x.Person.PersonDepartment == PersonDepartment.Directing)
