...
public static class ExternalApiExtensions
{
    public static IServiceCollection AddExternalApis(this IServiceCollection services, IConfiguration config)
    {
        ...
        services.AddRefitClient<ITmdbApi>(refitSettings)
            .ConfigureHttpClient((sp, client) => client.BaseAddress = sp.GetRequiredService<IOptions<TmdbConfig>>().Value.BaseUrl)
        ...
        services.AddRefitClient<IWatchmodeApi>(refitSettings)
            .ConfigureHttpClient((sp, client) => client.BaseAddress = sp.GetRequiredService<IOptions<WatchmodeOptions>>().Value.BaseUrl)
